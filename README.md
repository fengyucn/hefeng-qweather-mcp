...
一个基于 Model Context Protocol (MCP) 的和风天气服务，提供天气预报、气象预警、空气质量、历史数据、天文信息、热带气旋 (台风) 等多种气象数据查询功能。

## 功能特性

- **天气预报**: 获取3-30天详细天气预报（支持3d/7d/10d/15d/30d）
- **GeoAPI服务**: 热门城市查询、POI搜索和范围搜索功能
- **空气质量扩展**: 空气质量小时/日预报、监测站污染物数据查询
- **格点天气**: 全球任意坐标高分辨率数值预报天气（3-5公里分辨率）
- **热带气旋 (台风)**: 台风列表、实况路径和预报路径查询（支持西北太平洋流域）
- **气象预警**: 查询实时气象灾害预警信息
...
```python
# 获取热门城市列表
get_top_cities(10, "cn")  # 获取10个中国热门城市

# POI关键词搜索（米制半径）
search_poi("116.41,39.92", "故宫", "scenic", radius=10000)

# POI范围搜索（公里制半径）
search_poi_range("116.41,39.92", "scenic", radius=2)  # 2公里范围内景点

# 搜索餐厅和其他类型POI
search_poi("北京", "火锅", "scenic", radius=5000)  # 北京5公里内的景点
```
...
```python
# 空气质量小时预报
get_air_quality_hourly("39.92,116.41", "24h")  # 24小时逐小时空气质量
get_air_quality_hourly("39.92,116.41", "72h")  # 72小时逐小时空气质量

# 空气质量每日预报
get_air_quality_daily("39.92,116.41", "3d")     # 3天空气质量预报

# 监测站数据查询
get_air_quality_stations("P58911")  # 查询监测站P58911的污染物数据
get_air_quality_stations("P53763")  # 查询监测站P53763的污染物数据
```
...
# 获取2020年西北太平洋台风列表
get_storm_list("2020")
# 返回: {"code": "200", "storm": [{"id": "NP2008", "name": "巴威", ...}]}

# 获取指定台风的实况和路径
get_storm_track("NP2018")
# 返回: {"code": "200", "isActive": "1", "now": {...}, "track": [...]}

# 获取指定台风的预报路径
get_storm_forecast("NP2018")
# 返回: {"code": "200", "forecast": [{...}]}

# 注意：当前仅支持 basin=NP（西北太平洋），其他流域暂不可用
# 台风列表仅支持本年度和上一年度查询
```
...
#### GeoAPI工具（新功能）
- **`get_top_cities`** - 获取热门城市列表
  - 参数：`number`（返回数量，默认10）、`city_type`（城市类型：cn/world/overseas，默认cn）
  - 返回：热门城市列表，包含ID、坐标、时区等信息

- **`search_poi`** - POI关键词搜索
  - 参数：`location`（位置）、`keyword`（搜索关键词）、`poi_type`（POI类型：scenic/TSTA）、`radius`（搜索半径，米制，100-50000）
  - 返回：搜索到的兴趣点列表，包含详细位置信息

- **`search_poi_range`** - POI范围搜索
  - 参数：`location`（中心坐标）、`poi_type`（POI类型：scenic/TSTA）、`radius`（搜索半径，公里制，1-50）
  - 返回：指定范围内的兴趣点，按距离排序

#### 空气质量扩展工具（新功能）
- **`get_air_quality_hourly`** - 获取空气质量小时预报
  - 参数：`location`（纬度,经度坐标）、`hours`（预报时长：24h/72h/168h）、`lang`（语言，默认zh）
  - 返回：逐小时空气质量数据，包含AQI、污染物浓度、健康建议

- **`get_air_quality_daily`** - 获取空气质量每日预报
  - 参数：`location`（纬度,经度坐标）、`days`（预报天数：3d）、`lang`（语言，默认zh）
  - 返回：每日空气质量预报，包含AQI趋势和主要污染物

- **`get_air_quality_stations`** - 获取空气质量监测站数据
  - 参数：`station_id`（监测站ID，如P58911）、`lang`（语言，默认zh）
  - 返回：监测站污染物浓度数据，包含各类污染物的实时浓度值
...
#### 热带气旋 (台风) 工具（新功能）
- **`get_storm_list`** - 获取台风列表
  - 参数：`year`（年份，如 '2020'，仅支持本年度和上一年度）、`basin`（流域，默认 'NP' 西北太平洋）
  - 返回：台风列表，包含台风ID、名称、流域、年份、是否活跃等信息

- **`get_storm_track`** - 获取台风实况和路径
  - 参数：`stormid`（台风ID，可通过 `get_storm_list` 获取，如 'NP2018'）
  - 返回：台风实时位置、等级、气压、风速、历史轨迹路径
  - 注意：如台风已结束，`now` 字段可能为 null

- **`get_storm_forecast`** - 获取台风预报路径
  - 参数：`stormid`（台风ID，如 'NP2018'）
  - 返回：台风预报位置、等级、气压、风速等
  - 注意：如台风已结束，预报数据可能为空，建议先通过 `get_storm_list` 获取台风状态
...
- ✅ 新增 `get_storm_track`: 台风实况和路径（实时位置、等级、气压、风速、历史轨迹）
- ✅ 新增 `get_storm_forecast`: 台风预报路径（预测位置、等级、气压、风速）
- ✅ 支持 NP 西北太平洋流域，当前仅支持本年度和上一年度查询
- ✅ 完善参数验证、错误处理和中文文档字符串
...
- ✅ 新增GeoAPI服务：热门城市查询、POI搜索和范围搜索
- ✅ 新增空气质量扩展：小时/日预报、监测站污染物数据查询
- ✅ POI搜索支持关键词和范围两种模式，支持景点和潮汐站点类型
- ✅ 空气质量预报支持24/72/168小时逐小时预报和3天日预报
- ✅ 监测站数据提供实时污染物浓度值，支持精确的监测站ID查询
- ✅ 修复API参数格式和文档不一致问题，统一参数验证机制
